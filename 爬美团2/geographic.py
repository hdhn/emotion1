...
coordinate_train = pd.read_csv('./coos_train.csv', sep=",")
print(coordinate_train)
data = np.array(coordinate_train.groupby(['user']),dtype=object)
print(data)
geographic_data = pd.read_csv('poi_coos.txt', sep="\t",header=None)
# print(data[1])
...
user_poi_geographic_score_matrix = np.zeros((3, number1), np.float16)  # 构造已知用户数和商品数的元素全为0的矩阵
...
def geographic_score(t):
    ...
    np.savetxt('geographic_probability.csv', user_poi_geographic_score_matrix, fmt="%.08f",delimiter=',')
