...
def read_lines(filename):
    ...
    fp.close()
    return lines
posdict = read_lines("positive.txt")
negdict = read_lines("negative.txt")
# 程度副词词典
mostdict = read_lines('most.txt')   # 权值为2
verydict = read_lines('very.txt')   # 权值为1.5
moredict = read_lines('more.txt')   # 权值为1.25
ishdict = read_lines('ish.txt')   # 权值为0.5
#insufficientdict = tp.read_lines('insufficiently.txt')  # 权值为0.25
inversedict = read_lines('inverse.txt')  # 权值为-1
...
def single_review_sentiment_score(weibo_sent):
    ...
    cuted_review = cut_sentence(weibo_sent)  # 句子切分，单独对每个句子进行分析
...
def run_score():
    fp_test = open(r'打分测试.txt', 'rb')   # 待处理数据
    contents = []
...
def write_results(results):
    fp_result = open('test_result.txt', 'w',encoding='utf-8')
